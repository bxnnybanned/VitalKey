...
import { loginInventoryKeeper } from "../api/inventoryApi";
...
export default function Login() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const data = await loginInventoryKeeper(form);
      localStorage.setItem("inventory_token", data.access_token);
      localStorage.setItem("inventory_keeper", JSON.stringify(data.keeper));
      navigate("/dashboard");
    } catch (err) {
      setError(
    ...
    <div className="flex min-h-screen items-center justify-center bg-gradient-to-br from-blue-100 via-white to-cyan-100 px-4">
      <div className="w-full max-w-md rounded-[28px] border border-white/70 bg-white/90 p-8 shadow-2xl shadow-slate-200/60 backdrop-blur">
        <div className="mb-8 text-center">
          ...
          <p className="mt-2 text-gray-500">Inventory System Login</p>
        </div>
